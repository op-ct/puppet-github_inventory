...
#
plan github_inventory::update_mirrored_forks(
  ...
  Boolean $update_default_branch = true,
  Boolean $push_updates = true,
  Array[String,0] $prep_only_repos = ['puppetlabs-concat']
){
  ...
  $results = $github_repos.map |$repo| {
    ...
    if $forge_org {
      ...
      unless $mod_forge_org == $forge_org {
        warning("Module org ('${mod_forge_org}') does not match forge org '${forge_org}'; skipping")
        next(false)
      }
    }

    # The list all repos API call that provided the repo facts doesn't include parent repo info, so we query it here
    ...
    if $parent_info['archived'] {
      ...
      unless $dry_run {  debug::break() }  # TODO log
    }
    ...
    if $repo.name in $prep_only_repos {
      out::message( "${repo.name}: PREP ONLY REPO; not applying changes" )
      next( Result.new( $repo, $git_pull_result[0].value + {
        'extra' => $result_data + { 'skipped?' => 'repo marked as prep-only' }
      }))
    }

    if $dry_run {
      out::message( "DRY RUN: not applying changes" )
      next( Result.new( $repo, $git_pull_result[0].value + {
        'extra' => $result_data + { 'skipped?' => 'dry-run' }
      }))
    }

    ###out::message( "\n== Pause before push --follow-tags to ${repo.name} origin (${repo.facts['html_url']})")
    ###debug::break()
    ...
    ###out::message( "\n== Pause before changing origin default branch from '$rdb' to '$pdb' (${repo.facts['html_url']})")
    ###debug::break()
